...
def timestep_collision():
    ...
    #  ------------------- Collision  ------------------- #
    if tm.length(x[i] - obstacle.ball_center[0]) < ball_radius + contact_eps:
        normal = tm.normalize(x[i] - obstacle.ball_center[0])
        v[i] -=  tm.min(0, tm.dot(v[i], normal)) * normal
